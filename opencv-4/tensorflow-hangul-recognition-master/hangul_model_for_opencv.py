...
def export_model(model_output_dir, input_node_names, output_node_name):
    ...
    optimized_graph_file = os.path.join(
        model_output_dir, "optimized_" + MODEL_NAME + ".pb"
    )
...
def main(label_file, tfrecords_dir, model_output_dir, num_train_epochs):
    ...
    train_dataset = (
        tf.data.TFRecordDataset(train_data_files)
        .map(_parse_function)
        .shuffle(1000)
    ...
    x = tf.placeholder(
        tf.float32, [None, IMAGE_WIDTH * IMAGE_HEIGHT], name=input_node_name
    )
    ...
    x_conv3 = tf.nn.conv2d(h_pool2, W_conv3, strides=[1, 1, 1, 1], padding="SAME")
    ...
    y = tf.matmul(h_fc1, W_fc2) + b_fc2  # Do not use dropout!!
    ...
    cross_entropy = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(
            labels=tf.stop_gradient(y_), logits=y
        )
    )
    ...
    with tf.Session() as sess:
        ...
        tf.train.write_graph(
            sess.graph_def, model_output_dir, MODEL_NAME + ".pbtxt", True
        )
        ...
        try:
            ...
            while True:
                ...
                sess.run(
                    train_step,
                    feed_dict={x: train_images, y_: train_labels, keep_prob: 0.5},
                )
                if step % 100 == 0:
                    ...
                    print(
                        "Step %d, Training Accuracy %g" % (step, float(train_accuracy))
                    )
        ...
        test_dataset = (
            tf.data.TFRecordDataset(test_data_files)
            .map(_parse_function)
            .batch(BATCH_SIZE)
            .prefetch(1)
        )
        ...
        with gfile.FastGFile("./korean_recognition.pb", "wb") as f:
            f.write(output_graph_def.SerializeToString())
        print("korean_recognition.pb file is created!")
...
if __name__ == "__main__":
    ...
    main(args.label_file, args.tfrecords_dir, args.output_dir, args.num_train_epochs)
