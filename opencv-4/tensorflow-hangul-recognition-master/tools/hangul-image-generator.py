...
# 파워쉘(cmd) 에서 파일 실행시키기 위한 코드 :
# python tools\hangul-image-generator.py --label-file labels\256-common-hangul.txt --font-dir fonts --output-dir image-data
...
def generate_hangul_images(label_file, fonts_dir, output_dir):
    ...
    labels_csv = io.open(
        os.path.join(output_dir, "labels-map.csv"), "w", encoding="utf-8"
    )
        ...
        for font in fonts:
            ...
            for i in range(DISTORTION_COUNT):
                ...
                distorted_array = elastic_distort(
                    arr, alpha=random.randint(30, 36), sigma=random.randint(5, 6)
                )
...
def elastic_distort(image, alpha, sigma):
    ...
    dx = (
        gaussian_filter((random_state.rand(*shape) * 2 - 1), sigma, mode="constant")
        * alpha
    )
    dy = (
        gaussian_filter((random_state.rand(*shape) * 2 - 1), sigma, mode="constant")
