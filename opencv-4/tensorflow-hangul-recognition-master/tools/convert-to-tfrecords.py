...
class TFRecordsConverter(object):
    ...
    def __init__(
        self, labels_csv, label_file, output_dir, num_shards_train, num_shards_test
    ):
    ...
    def convert(self):
        ...
        for i in range(1, self.num_shards_train):
            shard_path = os.path.join(
                self.output_dir, "train-{}.tfrecords".format(str(i))
            )
        ...
        final_shard_path = os.path.join(
            self.output_dir, "train-{}.tfrecords".format(str(self.num_shards_train))
        )
        ...
        for i in range(1, self.num_shards_test):
            shard_path = os.path.join(
                self.output_dir, "test-{}.tfrecords".format(str(i))
            )
            file_indices = np.arange(start, start + files_per_shard, dtype=int)
            start = start + files_per_shard
            self.write_tfrecords_file(shard_path, file_indices)
        ...
        final_shard_path = os.path.join(
            self.output_dir, "test-{}.tfrecords".format(str(self.num_shards_test))
        )
        self.write_tfrecords_file(final_shard_path, file_indices)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="File containing image paths and corresponding " "labels.",
    )
    parser.add_argument(
        "--label-file",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Number of shards to divide training set " "TFRecords into.",
    )
    parser.add_argument(
        "--num-shards-test",
        type=int,
        ...
        help="Number of shards to divide testing set " "TFRecords into.",
    )
