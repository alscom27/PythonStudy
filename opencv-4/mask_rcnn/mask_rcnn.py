...
def drawBox(img, classId, conf, left, top, right, bottom):
    ...
    cv2.rectangle(
        img,
        (left - 1, top - labelSize[1] - baseLine),
        (left + labelSize[0], top),
        colors[classId],
        -1,
    )
    cv2.putText(
...
class_labels = "./coco_90.names"
confThreshold = 0.6
maskThreshold = 0.3

# 테스트 이미지 파일
img_files = ["../images/dog.jpg", "../images/traffic.jpg", "../images/sheep.jpg"]
...
for f in img_files:
    ...
    for i in range(numDetections):
        ...
        if score > confThreshold:
            ...
            img[y1 : y2 + 1, x1 : x2 + 1][mask] = (
                0.7 * colors[classId] + 0.3 * roi
            ).astype(np.uint8)
